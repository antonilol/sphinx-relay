...
export function keysend(
  ...
  sphinxLogger.info(['keysend', opts.dest], logging.Lightning)
    ...
    try {
      ...
      if (isProxy(lightning)) {
        ...
        if (isGL(lightning)) {
          ...
          // TODO wrong buffer type: Type 'Buf' is not assignable to type 'string | Buffer | Uint8Array | undefined'.
...
export async function listChannels(
  ...
  return new Promise(async (resolve, reject) => {
    ...
    if (isGL(lightning)) {
      ...
      // TODO proxy?
      (<LightningClient>lightning).listChannels(opts, function (err, response) {
        if (err == null && response) {
          resolve(interfaces.listChannelsResponse(response))
        } else {
          reject(err)
